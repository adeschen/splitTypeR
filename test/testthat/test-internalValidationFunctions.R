### Unit tests for internatlValidationFunctions functions
...
test_that("validateRunSubtyping() must return error when geneLists is a character string", {

    error_message <- "The \'geneLists\' object must be a list with a least 2 entries."

    expect_error(splitTypeR:::validateRunSubtyping(geneLists='test', 
        expectedCountsMatrix=matrix(data=rep(3, 6), nrow = 2)), 
        error_message)
})

test_that("validateRunSubtyping() must return error when geneLists is a list with 1 entry", {
    ...
    expect_error(splitTypeR:::validateRunSubtyping(geneLists=gList, 
        expectedCountsMatrix=matrix(data=rep(3, 6), nrow = 2)), 
        error_message)
})
...
test_that("validateRunSubtyping() must return error when gds is a character string", {
